...
export default async function Home() {
  ...
  const { data: trips } = await supabase.from('trips').select('revenue_eur');
  const { data: expenses } = await supabase.from('trip_expenses').select('amount_eur');
    ...
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <Card className="bg-white shadow-sm border-0">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-gray-500">Всего расходов</CardTitle>
            </CardHeader>
            <CardContent>
